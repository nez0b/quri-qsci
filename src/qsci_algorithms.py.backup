...
class TimeEvolvedQSCI(QSCIBase):
    ...
    def _create_time_evolved_state(
        ...
        try:
            ...
            print(f"Warning: quri-algo not available ({e}), using simplified Trotter implementation")
            if trotter_steps is None:
    ...
    def _add_fallback_trotter_step(self, circuit: QuantumCircuit, dt: float) -> None:
        ...
        # This is a simplified fallback - just add some rotation gates
        # In practice, would need full Pauli evolution implementation
        for qubit in range(circuit.qubit_count):
            # Add small rotations as approximation to Hamiltonian evolution
            circuit.add_RZ_gate(qubit, float(dt * 0.1))  # Simplified Z evolution
            if qubit < circuit.qubit_count - 1:
                # Add entangling gates between adjacent qubits
                circuit.add_CNOT_gate(qubit, qubit + 1)
                circuit.add_RZ_gate(qubit + 1, float(dt * 0.05))  # Interaction term
                circuit.add_CNOT_gate(qubit, qubit + 1)
