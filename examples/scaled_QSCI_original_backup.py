#!/usr/bin/env python3
"""
BACKUP OF ORIGINAL SCALED_QSCI.PY IMPLEMENTATION
=================================================
Created: 2025-01-05
Purpose: Preserve original implementation before fixing critical bugs

This is a backup of the original scaled_QSCI.py implementation that contained
critical bugs causing >1 Ha energy errors. Preserved for reference and potential
reversion during the debugging process.

Known Issues in this version:
1. System definition mismatch: (10e, 14o) vs (10e, 18o)
2. State representation bug in determinants_to_qubit_states()
3. Hamiltonian matrix construction errors
4. No validation against PySCF reference calculations

See scaled_QSCI.md for detailed bug analysis and comparison with working
...
2. N2/cc-pVDZ molecular system with (10e, 14o) active space
...
class ScaledQSCIConfig:
    ...
    bond_length: float = 1.1  # Angstrom
    basis: str = "cc-pVDZ"
    n_electrons: int = 10
    n_orbitals: int = 18
...
class ScaledQSCI(QSCIBase):
    ...
    def run_with_ci_coefficients(self, ci_coefficients: np.ndarray, 
        ...
        # STEP 1: Extract actual CI determinants from CASCI solver
        print(f"  Extracting top {num_determinants} determinants from CASCI...")
        all_determinants = extract_ci_determinants(self.cas_solver, num_determinants=None)
        
        # STEP 2: Apply α-scaling to the determinant probabilities
        print(f"  Applying α = {self.alpha} scaling to determinant probabilities...")
        original_probs = [abs(det[0])**2 for det in all_determinants]
        
        if self.alpha == 0.0:
            scaled_probs = np.ones_like(original_probs)
        else:
            scaled_probs = np.array(original_probs)**self.alpha
        
        # Normalize scaled probabilities
        scaled_probs /= np.sum(scaled_probs)
        
        # Select top determinants based on scaled probabilities
        top_indices = np.argsort(scaled_probs)[::-1][:num_determinants]
        selected_determinants = [all_determinants[i] for i in top_indices]
        
        print(f"  Selected {len(selected_determinants)} determinants with α-scaling")
        
        # STEP 3: Convert determinants to qubit computational basis states
        ...
        # STEP 4: Create ComputationalBasisState objects for QSCI
        selected_states = []
        ...
        # STEP 5: Generate and diagonalize truncated Hamiltonian
...
def debug_casci_structure(cas_solver):
    ...
    if hasattr(cas_solver, 'nelecas'):
        ...
        na = nb = 5  # For N2 (10e, 14o) this would be (5,5)
    ...
    if hasattr(cas_solver, 'ncas'):
        ...
        norb = 14  # Fallback for our N2 system
...
def extract_ci_determinants(cas_solver, num_determinants=None):
    """Extract actual Slater determinants from CASCI CI coefficients.
    
    This function correctly maps CI coefficient indices to actual determinant configurations
    using PySCF's cistring module, fixing the fundamental flaw in the previous implementation.
    ...
    Returns:
        List of tuples [(coeff, alpha_string, beta_string), ...] sorted by |coeff|
    """
    try:
        from pyscf.fci import cistring
    except ImportError:
        raise ImportError("PySCF cistring module required for determinant extraction")
    
    print(f"\n=== EXTRACTING CI DETERMINANTS ===")
    ...
    if hasattr(cas_solver, 'ncas'):
        ...
        norb = 14  # Fallback for N2 system
    
    print(f"Active space: ({na}α, {nb}β) electrons in {norb} orbitals")
    
    # Generate alpha and beta strings
    ...
    # Extract CI coefficients and map to determinants
    ci_coeffs = cas_solver.ci.flatten() if hasattr(cas_solver.ci, 'flatten') else cas_solver.ci
    
    determinants = []
    ci_index = 0
    
    for i, alpha_str in enumerate(alpha_strings):
        for j, beta_str in enumerate(beta_strings):
            if ci_index < len(ci_coeffs):
                coeff = ci_coeffs[ci_index]
                if abs(coeff) > 1e-12:  # Only keep significant coefficients
                    determinants.append((coeff, alpha_str, beta_str, ci_index))
                ci_index += 1
    
    # Sort by coefficient magnitude
    determinants.sort(key=lambda x: abs(x[0]), reverse=True)
    
    print(f"Found {len(determinants)} significant determinants")
